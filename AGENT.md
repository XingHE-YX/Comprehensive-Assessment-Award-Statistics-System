...
- 班级口令和管理员密码只保存 Argon2id 哈希。修改码同时保存 Argon2id 验证哈希和按申报编号绑定的认证加密密文；仅一次性成功页和受管理员保护的详情页可显示明文，不进入列表、普通学生详情、Excel、日志或 URL。
- 修改码密钥来自持久 SESSION_SECRET，必须随数据安全备份；轮换须迁移密文。旧记录没有密文时原码不可恢复，禁止自动重置；管理员手动重置须 POST、CSRF、明确确认和版本条件写入，旧码与旧学生会话失效，审核和附件不变。
